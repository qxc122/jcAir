...
#pragma mark - 创建对象-->>带button skipbutton
+(instancetype)sharedWithImages:(NSArray *)imageNames buttonImage:(NSString *)buttonImageName buttonFrame:(CGRect)frame  skipbuttonImage:(NSString *)skipbuttonImageName skipbuttonFrame:(CGRect)skipframe{
    images = imageNames;
    isScrollOut = NO;
    enterBtnFrame = frame;
    enterBtnImage = buttonImageName;
    skipBtnFrame = skipframe;
    skipBtnImage = skipbuttonImageName;
    launch = [[LaunchIntroductionView alloc] initWithFrame:CGRectMake(0, 0, kScreen_width, kScreen_height)];
    launch.backgroundColor = [UIColor whiteColor];
    return launch;
}
...
-(void)createScrollView{
    ...
    if (skipBtnImage.length) {
        UIButton *enterButton = [[UIButton alloc] initWithFrame:skipBtnFrame];
        [enterButton setImage:[UIImage imageNamed:skipBtnImage] forState:UIControlStateNormal];
    ...
    for (int i = 0; i < images.count; i ++) {
        ...
        if (i == images.count - 1) {
            ...
            if (!isScrollOut) {
                ...
                [enterButton setWithNormalColor:0xFFFFFF NormalAlpha:1.0 NormalTitle:NSLocalizedString(@"enter", nil) NormalImage:nil NormalBackImage:nil SelectedColor:0x0 SelectedAlpha:0 SelectedTitle:nil SelectedImage:nil SelectedBackImage:nil Font:17 NormalROrM:@"r" backColor:0x0 backAlpha:0];
                [enterButton SetFilletWith:35.f];
                [enterButton SetBordersWith:0.5 Color:ColorWithHex(0xFFFFFF, 1.0)];
            }
        }
    }
    CGFloat yy = 137;
    ...
    page = [[KTPageControl alloc] initWithFrame:CGRectMake(0, 137, kScreen_width, 15)];
    page.numberOfPages = images.count;
    page.backgroundColor = [UIColor clearColor];
    
    page.pageIndicatorTintColor =self.currentColor;
    page.currentPageIndicatorTintColor = self.nomalColor;
